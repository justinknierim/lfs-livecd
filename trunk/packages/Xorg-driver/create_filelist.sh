...
HOME=$(pwd)
URL="http://anduin.linuxfromscratch.org/files/BLFS/svn/xorg/driver-7.4.wget"
TMP=$(mktemp -d)
FILE=$(basename $URL)
cd $TMP && wget -q $URL
...
do
	wget -q http://kerrek.linuxfromscratch.org/pub/blfs/conglomeration/Xorg-driver/$file
	echo '' >> filelist
